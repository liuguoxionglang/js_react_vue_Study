(function (global, factory) {
    ...
    function createStore(reducer, preloadedState, enhancer) {
     ...
     /*reducer必须是一个函数，通常都是纯函数*/
      ...
      var currentState = preloadedState; // 就是传入的初始state嘛
      ...
      // 与getState()函数对应，修改redux中state的状态，redux中修改state，只能使用dispacth函数发起,
      // 传达一个action给reducer，reducer会根据action和currentState以及自己的内部实现逻辑，来计算出新的state，
    ...
    // 这个函数分为两部分，第一部分是检验用户传入的reducers的准确性。第二部分就是计算state的逻辑。
    ...
    function compose() {
      ...
      /*当数组中无函数对象时，默认返回一个一个函数，此函数输入和输出一样*/
      if (funcs.length === 0) {
        return function (arg) {
          return arg;
        };
      ...
      /*当数组的中中间件函数长度不止一个的时候，利用es6数组的reduce函数，返回多个中间件函数嵌套的函数*/
